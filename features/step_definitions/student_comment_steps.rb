...
Given(/the following comments exist:/) do |comments_table|
  comments_table.hashes.each do |comment|
    p_id = comment['project'].eql?('nil') ? nil : Project.find_by(name: comment['project']).id
    st_id = comment['student_task'].eql?('nil') ? nil : StudentTask.find_by(title: comment['student_task']).id
    i_id = comment['iteration'].eql?('nil') ? nil : Iteration.find_by(name: comment['iteration']).id

    Comment.create!(metric_sample_id: comment["metric_sample"],
...
Given(/the following metrics samples exist:/) do |metrics_table|
  ...
  metrics_table.hashes.each do |metric|
    MetricSample.create!(metric_name: metric["metric_name"],
                         project_id: Project.find_by(name: metric["project"]).id,
                         score: metric["score"],
                         image: File.read('./db/fake_data/slack2.json'), #TODO: seems very hard-coded
                         created_at: d)
